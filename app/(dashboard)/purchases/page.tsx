...
async function getPurchases() {
  try {
    ...
    return transactions.filter((transaction: Transaction) => transaction.type === "purchase");
  } catch (error) {
...
export default async function PurchasesPage() {
  ...
  return (
    <DashboardShell>
      <DashboardHeader 
        heading="Purchases" 
        text="View and manage all your purchase transactions."
      >
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Complete list of all purchases made from suppliers
          </CardDescription>
        </CardHeader>
        <CardContent>
          {purchases.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <p className="text-muted-foreground mb-4">No purchase transactions found</p>
              <Link href="/purchases/new">
                <Button variant="outline">Record your first purchase</Button>
              </Link>
            </div>
          ) : (
